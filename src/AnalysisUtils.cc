...
bool SelectBestScoreBJets2( TreeVars outtreeVars,int &bjet_lead_i,int &bjet_sublead_i,const bool &useMTD )
{
  ...
  int ijet1=0;
  if(outtreeVars.jet_mvav2[0]>=5)
    outtreeVars.jet_mvav2[0]=1;
  else
    outtreeVars.jet_mvav2[0]=0;

  int btag1 = outtreeVars.jet_mvav2[0];
  float pt1 = outtreeVars.jet_pt[0];
  //---------------------------------------------------------------
  //cout<<"0\tbtag="<<btag1<<"\tpt="<<pt1<<endl;
  //---------------------------------------------------------------
  for(int i=1; i<outtreeVars.nJets; i++)
  {
    if(outtreeVars.jet_mvav2[i]>=5)
      outtreeVars.jet_mvav2[i]=1;
    else
      outtreeVars.jet_mvav2[i]=0;
    //---------------------------------------------------------------
    //cout<<i<<"\tbtag="<<outtreeVars.jet_mvav2[i]<<"\tpt="<<outtreeVars.jet_pt[i]<<endl;
    //---------------------------------------------------------------
    if(outtreeVars.jet_mvav2[i] > btag1)
    {
      ijet1=i;
      btag1=outtreeVars.jet_mvav2[i];
  ...
  for(int i=0; i<outtreeVars.nJets; i++)
  {
    ...
    {
      ijet2=i;
      btag2=outtreeVars.jet_mvav2[i];
      pt2 = outtreeVars.jet_pt[i];
    }
    else
      if(outtreeVars.jet_mvav2[i] > btag2)
      {
	ijet2=i;
	btag2=outtreeVars.jet_mvav2[i];
	pt2 = outtreeVars.jet_pt[i];
...
bool SelectBestScoreBJets3(const TreeVars &outtreeVars,int &bjet_lead_i,int &bjet_sublead_i )//jet selection implementing new btag
{
...
{
  float gen_eta, gen_phi, gen_E;
  TLorentzVector H1, H2;
  
  int i=0;
  while(i<treeVars.N_GenPart && H2.E()==0)
  {
    if(treeVars.GenPart_pid[i]==25)
    {
      ...
      {
	if(H1.E()==0)
	  H1.SetPtEtaPhiE(treeVars.GenPart_pt[i], treeVars.GenPart_eta[i], treeVars.GenPart_phi[i], gen_E);
	else
	  if(H2.E()==0)
	    H2.SetPtEtaPhiE(treeVars.GenPart_pt[i], treeVars.GenPart_eta[i], treeVars.GenPart_phi[i], gen_E);
      }
    }
    ++i;
  }
  ...
  outtreeVars.mHH_gen = HH.M();
  
  TLorentzVector boosted_H1(H1);
  boosted_H1.Boost( -HH.BoostVector() );
  outtreeVars.costhetaHH_gen = fabs(boosted_H1.CosTheta());

  //TLorentzVector boosted_H2(H2);
  //boosted_H2.Boost( -HH.BoostVector() );
  //float costheta_HH2 = boosted_H2.CosTheta();
